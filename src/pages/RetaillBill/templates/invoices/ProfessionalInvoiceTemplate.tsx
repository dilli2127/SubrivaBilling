 ...
 */
const ProfessionalInvoiceTemplate: React.FC<
...
> = ({ billData, settings }) => {
  ...
  const discountValue = Number(
    billData?.discountValue || billData?.discount_value || 0
  );
  const discountAmount =
    discountValue > 0
      ? discountValue
      : billData?.discount_type === 'percentage'
        ? (valueOfGoods * discount) / 100
        : discount;

  // For display purposes - calculate item totals
  const itemsTotal =
    billData?.items?.reduce((sum: number, item: any) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {(billData?.organisationItems?.logo_url ||
        ...
        >
          {isGstInclusive ? '⚠️ Tax Inclusive' : 'ℹ️ Tax Exclusive'} - Prices{' '}
          {isGstInclusive ? 'include' : 'exclude'} GST
        </div>
      </div>
        ...
        <div style={{ flex: 1, border: '1px solid #000', padding: 8 }}>
          ...
          <div style={{ fontSize: 10, lineHeight: 1.6 }}>
            {billData?.customerCompanyName && (
              <div>
                <strong>Company:</strong> {billData.customerCompanyName}
              </div>
            )}
            {/* <div>
            ...
            {billData?.customerAddress && (
              <div>
                <strong>Address:</strong> {billData.customerAddress}
              </div>
            )}
            ...
            {billData?.customer_pan && (
              <div>
                <strong>PAN:</strong> {billData.customer_pan}
              </div>
            )}
            {billData?.customerPhone && (
              <div>
                <strong>Mobile:</strong> {billData.customerPhone}
              </div>
            )}
            {billData?.customerEmail && (
              <div>
                <strong>Email:</strong> {billData.customerEmail}
              </div>
            )}
          </div>
        </div>
        ...
        <tbody>
          {Array.isArray(billData?.items) && billData.items.length > 0 ? (
            <>
              ...
              {[...Array(Math.max(0, 3 - billData.items.length))].map(
                (_, idx) => (
                  <tr key={`empty-${idx}`}>
                    <td style={tdStyle}>&nbsp;</td>
                    <td style={tdStyle}>&nbsp;</td>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 12 }}>
        ...
        <div style={{ width: 220 }}>
          ...
          <div style={{ marginBottom: 3 }}>
            ...
            >
              <span style={{ fontSize: 10 }}>
                <strong>{isGstInclusive ? 'CGST (Incl):' : 'CGST:'}</strong>
              </span>
          ...
          <div style={{ marginBottom: 3 }}>
            ...
            >
              <span style={{ fontSize: 10 }}>
                <strong>{isGstInclusive ? 'SGST (Incl):' : 'SGST:'}</strong>
              </span>
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          {settings?.show_terms_on_invoice && settings?.invoice_terms && (
            <div
              style={{ border: '1px solid #000', padding: 8, marginBottom: 8 }}
            >
              <div
                style={{ fontWeight: 'bold', marginBottom: 6, fontSize: 10 }}
              >
                Terms and Conditions
              </div>
              <div
                style={{ fontSize: 9, lineHeight: 1.5, whiteSpace: 'pre-wrap' }}
              >
            ...
            <div style={{ border: '1px solid #000', padding: 8 }}>
              ...
              <div style={{ fontSize: 9, lineHeight: 1.5 }}>
                {settings?.account_holder_name && (
                  <div>
                    <strong>Name:</strong> {settings.account_holder_name}
                  </div>
                )}
                {settings?.account_number && (
                  <div>
                    <strong>A/c No:</strong> {settings.account_number}
                  </div>
                )}
                {settings?.bank_name && (
                  <div>
                ...
                {settings?.upi_id && (
                  <div>
                    <strong>UPI ID:</strong> {settings.upi_id}
                  </div>
                )}
              </div>
            </div>
        ...
        >
              ...
              settings?.qrCodeDataUrl ? (
                ...
                >
                  ...
                  <img
                    ...
                    style={{
                      display: 'block',
                      width: settings?.qr_size || 100,
                      height: settings?.qr_size || 100,
                      margin: '0 auto',
                    }}
                  />
                  {settings?.show_upi_id_text && settings?.upi_id && (
                    <div style={{ fontSize: 8, marginTop: 4, color: '#333' }}>
                      UPI: {settings.upi_id}
                    </div>
                  )}
                  <div style={{ fontSize: 7, marginTop: 2, color: '#666' }}>
          ...
          <div style={{ textAlign: 'center', marginTop: 'auto' }}>
            <div style={{ fontSize: 9, marginBottom: 20 }}>
              For{' '}
              {billData?.organisationItems?.org_name ||
                userItem?.organisationItems?.org_name ||
                'Company Name'}
